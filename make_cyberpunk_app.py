...
html_cyberpunk_template = f"""<!DOCTYPE html>
<html lang="ru">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>KOBYLA HAS WAKEN UP 2099 / КОБЫЛА ПРОСНУЛАСЬ 2099 - Cyberpunk VN</title>
    <style>
        ...
        * {{
        ...
        body {{
            ...
            align-items: center;
            background-image: 
                radial-gradient(circle at 50% 20%, rgba(255, 0, 51, 0.2) 0%, rgba(3, 2, 5, 0.98) 75%),
...
<body>
    ...
    <script>
        const storyData = {json.dumps(cyber_story_nodes, ensure_ascii=False, indent=2)};
        ...
        // Continuous Synthwave Background Music Generator
        class CyberpunkMusicEngine {{
            constructor() {{
                ...
                this.step = 0;
                this.tempo = 110;
                this.bassNotes = [55, 55, 65.4, 55, 73.4, 55, 43.65, 55];
                this.arpNotes = [220, 261.63, 329.63, 392.00, 440, 392.00, 329.63, 261.63];
            }}
            ...
            tick() {{
                ...
                // 1. Darksynth Bass
                if (step % 2 === 0) {{
                    const bassFreq = this.bassNotes[(step / 2) % this.bassNotes.length];
        ...
        const musicEngine = new CyberpunkMusicEngine();
        ...
        function updateUI() {{
            ...
            if (node.is_ending) {{
                ...
                restartBtn.innerText = currentLang === 'ru' ? ' Перезапустить новеллу 2099' : ' Reboot Cyberpunk VN';
        ...
        function makeChoice(choice) {{
            // Auto start music on first choice click if not playing!
            if (!musicEngine.isPlaying) {{
                toggleMusic();
            }}
...
with open("/home/user/kobyla_has_waken_up.html", "w", encoding="utf-8") as f:
    f.write(html_cyberpunk_template)

print("Overwritten kobyla_has_waken_up.html with Cyberpunk Post-Apoc edition!")
